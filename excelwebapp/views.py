...
def home(request):
	return render(request, 'excelwebapp/index.html')

def plan(request):
	plans= Plan.objects.all()
	context= {'plans':plans}
	return render(request, 'excelwebapp/plan.html', context)

def otherpayment(request):
    return render(request, 'excelwebapp/otherpayment.html')


def about(request):
	return render(request, 'excelwebapp/company.html')


def services(request):
	return render(request, 'excelwebapp/services.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				email= user.email
				template= render_to_string('excelwebapp/otpalert.html', {'otp':otp})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
					'Use this OTP code to login to your ApexoilHedge!',
					template,
	...
	context={}
	return render(request, 'excelwebapp/signin.html')
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if str(otp_obj.otp_code) == str(entered_otp) and not otp_obj.is_expired():
				email= user.email
				template= render_to_string('excelwebapp/loginAlert.html', {'name':user.username})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
				...
				return render(request, 'excelwebapp/verify_otp.html', {'form': form, 'error': 'Invalid or expired OTP'})
	else:
		form = OTPForm()

	return render(request, 'excelwebapp/verify_otp.html', {'form': form})


def privacy(request):
	return render(request, 'excelwebapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'excelwebapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('excelwebapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('excelwebapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to ApexoilHedge',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('excelwebapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with ApexoilHedge!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@ApexoilHedge.com'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'excelwebapp/signup.html', context)
...
def dashboard(request):
	...
	return render(request, 'excelwebapp/dashboard.html', context)
...
def admindashboard(request):
	...
	return render(request, 'excelwebapp/adminpage.html', context)


def terms(request):
	return render(request, 'excelwebapp/terms.html')


def realestate(request):
	return render(request, 'excelwebapp/realestate.html')


def crypto(request):
	return render(request, 'excelwebapp/crypto.html')


def forex(request):
	return render(request, 'excelwebapp/forex.html')


def nft(request):
	return render(request, 'excelwebapp/nft.html')


def contact(request):
	return render(request, 'excelwebapp/contact.html')


def stocks(request):
	return render(request, 'excelwebapp/stocks.html')


def agriculture(request):
	return render(request, 'excelwebapp/agriculture.html')


def gold(request):
	return render(request, 'excelwebapp/gold.html')


def retirement(request):
	return render(request, 'excelwebapp/retirement.html')
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.ApexoilHedge.com/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.ApexoilHedge.com/dashboard"
			})
			...
			template= render_to_string('excelwebapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@ApexoilHedge.com'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'excelwebapp/deposittwo.html', context)
...
def investmentPlans(request):
	plans= Plan.objects.all()
	context= {'plans':plans}
	return render(request, 'excelwebapp/investmentplans.html', context)

@login_required(login_url='signin')
def prospectorDeposit(request):
	if request.method == 'POST':
	...
	context= {}
	return render(request, 'excelwebapp/prospectorDepositForm.html', context)

@login_required(login_url='signin')
def UpstreamDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'excelwebapp/UpstreamDeposit.html', context)

@login_required(login_url='signin')
def PetrolMaxDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'excelwebapp/PetrolMaxDeposit.html', context)

@login_required(login_url='signin')
def premiumHedgeDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'excelwebapp/premiumHedgeDeposit.html', context)

@login_required(login_url='signin')
def eliteDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'excelwebapp/eliteDeposit.html', context)
...
def deposit(request):
	...
	if price_amount and pay_currency:
		...
		return redirect(generated_link)
		context={}
	return render(request, 'excelwebapp/deposit.html', context)
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@ApexoilHedge.com'])
	...
	context={}
	return render(request, 'excelwebapp/withdrawal.html', context)
...
def history(request):
	...
	return render(request, 'excelwebapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'excelwebapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'excelwebapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'excelwebapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'excelwebapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'excelwebapp/referralprofiles.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'excelwebapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('excelwebapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('excelwebapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'excelwebapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	template= render_to_string('excelwebapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'excelwebapp/account_settings.html', context)
...
def create_bonus(request):
	try:
		clients= Client.objects.all()
		if request.method == 'POST':
			client_instance= request.POST.get('bonus_client')
			amount= request.POST.get('amount')
			code= request.POST.get('code')
			print(client_instance)
			if client_instance and amount and code:
				try:
					client_id= Client.objects.get(id=client_instance)
					bonus_creation= Bonus.objects.create(
						client=client_id,
						amount= amount,
						bonus_code= code
					)
					if bonus_creation:
						#send email to client with bonus code
						client_email= client_id.user.email
						if client_email:
							#proceed to send email
							template= render_to_string('excelwebapp/bonus_email.html', {'amount':amount, 'code':code})
							plain_message= strip_tags(template)
							emailmessage= EmailMultiAlternatives(
								'Bonus Notification',
								plain_message,
							...
							return HttpResponse("This client did not receive a bonus code in their email")
					return HttpResponse('Bonus created successfully')
				except:
					return HttpResponse("Couldn't create bonus at this time. Please try again later")
			else:
				return HttpResponse("Please fill form correctly")
	except:
		return HttpResponse("Could'nt get client list")
	context={'clients':clients, }
	return render(request, 'excelwebapp/create_bonus.html', context)


@login_required(login_url='signin')
def use_bonus(request):
	if request.method == 'POST':
		client_request_code= request.POST.get('client_request_code')
		print(client_request_code)
		if client_request_code:
			try:
				code_info= Bonus.objects.get(bonus_code=client_request_code)
				if code_info.is_valid:
					#proceed to top up account and update code status
					amount= code_info.amount
					client_id= code_info.client.id
					client_bal= code_info.client.balance
					newclient_bal= float(client_bal) + float(amount)
					client_info= Client.objects.filter(id=client_id)
					client_info.update(balance=newclient_bal)
					code_info.is_valid = False
					code_info.save()
					client_email= code_info.client.email_address
					template= render_to_string('excelwebapp/usebonus_email.html', {'name':code_info.client})
					plain_message= strip_tags(template)
					emailmessage= EmailMultiAlternatives(
						'Bonus Code Used',
						plain_message,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					send_mail(code_info.client, "A client with username: {} has used their bonus code of {} usd. Money has been credited to their account".format(code_info.client, amount),settings.EMAIL_HOST_USER, ['info@ApexoilHedge.com'])
					return redirect('dashboard')
				else:
					return HttpResponse('Code has already been used')
			except:
				return HttpResponse('Failed to find bonus code. Please note that code is case sensitive')
		else:
			return HttpResponse('Please enter a code before submitting form')
	return render(request, 'excelwebapp/use_bonus.html')
...
def notifications(request):
	...
	return render(request, 'excelwebapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('excelwebapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('excelwebapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@ApexoilHedge.com'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'excelwebapp/transferFunds.html', context)
...
def card(request):
	if request.method == 'GET':
		...
		if form.is_valid():
			...
			print(name)
		try:
			send_mail("Apex oil Hedge Card Request", "User with name {} and email {} has requested for card to this address: {}".format(name, email, address),email, ['info@ApexoilHedge.com'])
			print('Request Sent')
			return HttpResponse('Your request has been sent successfully!')
		except:
			print('Request not sent')
			messages.error(request, 'Request Not sent, Try again later.')
		messages.success(request, 'Your request has been sent successfully')
	context={'form':form}
	return render(request, 'excelwebapp/card.html', context)
...
def video_page(request):
	...
	return render(request, 'excelwebapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('excelwebapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('excelwebapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@ApexoilHedge.com'])
			except BadHeaderError:
				pass
	...
	return render(request, 'excelwebapp/reinvest.html', context)
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@ApexoilHedge.com'])
	...
	return render(request, 'excelwebapp/loan.html')
...
def portfolio(request):
	...
	return render(request, 'excelwebapp/portfolio.html', context)